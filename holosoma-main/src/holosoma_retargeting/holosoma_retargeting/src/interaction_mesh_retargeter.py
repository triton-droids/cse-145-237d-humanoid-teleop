...
class InteractionMeshRetargeter:
    ...
    def __init__(
        ...
        if fix_orientation:
            # Optimize xyz translation (indices 0-2) + actuated joints (indices 7+)
            # but NOT the quaternion (indices 3-6). This prevents the optimizer from
            # tilting the torso to minimize interaction mesh cost.
            self.q_a_indices = np.concatenate([
                np.arange(3),  # xyz translation
                np.arange(7, 7 + self.task_constants.ROBOT_DOF),  # actuated joints
            ])
        else:
        ...
        # Apply manual bounds. Keys are qpos indices; only apply those present in q_a_indices.
        for key_str, val in self.task_constants.MANUAL_LB.items():
            qpos_idx = int(key_str)
            pos = np.where(self.q_a_indices == qpos_idx)[0]
            if len(pos) > 0:
                self.q_a_lb[pos[0]] = val
        for key_str, val in self.task_constants.MANUAL_UB.items():
            qpos_idx = int(key_str)
            pos = np.where(self.q_a_indices == qpos_idx)[0]
            if len(pos) > 0:
                self.q_a_ub[pos[0]] = val
    ...
    def _setup_visualization(self):
        ...
        # IMPORTANT: robot_base is a PARENT frame that carries the base transform (position + orientation).
        # ViserUrdf is attached as a CHILD so that update_cfg() cannot overwrite the base transform.
        self.robot_base = self.server.scene.add_frame("/world/robot_base", show_axes=False)
        ...
        # Create ViserUrdf instance for robot as a child of robot_base
        self.viser_robot = ViserUrdf(
            self.server,
            urdf_or_path=self.robot_urdf,
            root_node_name="/world/robot_base/robot",
        )

        # Similarly for object
        if self.object_model_path:
            self.object_base = self.server.scene.add_frame("/world/object_base", show_axes=False)
            ...
            # Create ViserUrdf instance for object as a child of object_base
            self.viser_object = ViserUrdf(
                self.server,
                urdf_or_path=self.object_urdf,
                root_node_name="/world/object_base/object",
            )
    ...
    def retarget_motion(
        ...
        if q_nominal_list is not None:
            ...
            # Set frame 0's full initial configuration first (ensures quaternion is
            # initialized correctly even when fix_orientation excludes it from q_a_indices).
            q_locked_list[0, : len(q_a_init)] = q_a_init
            # Then write only the optimizable DOF (may be a subset if fix_orientation=True).
        ...
        # Remove previous debug visualization (no-op if visualize is off: draw_keypoints returns None)
        if self.debug:
            for handle_list in (
            ...
            if apply_foot_sticking:
                ...
                for key, J_WF in J_WF_dict.items():
                    ...
                    if apply_left or apply_right:
                        ...
                        Jxy = J_WF[:2, :]  # (2 x nq_act) — already reduced to q_a_indices cols
                        constraints += [
                            Jxy @ dqa >= p_lb[:2],
                            Jxy @ dqa <= p_ub[:2],
                        ]
