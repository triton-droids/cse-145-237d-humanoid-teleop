...
def _compute_q_init_base(
    ...
    if task_type == "robot_only":
        if data_format == "lafan":
            ...
            # MuJoCo order: pos first, then quat
            q_joints_init = getattr(constants, "Q_INIT_JOINTS", np.zeros(constants.ROBOT_DOF))
            q_init_base = np.concatenate([human_joints[0, 0, :3], human_quat_init, q_joints_init])
    elif task_type == "object_interaction":
        _, human_quat_init = transform_from_human_to_world(
            human_joints[0, 0, :], object_poses[0], np.array([0.0, 0.0, 0.0])
        )
...
def build_retargeter_kwargs_from_config(
    ...
    kwargs = {
        ...
        "fix_orientation": retargeter_config.fix_orientation,
...
def main(cfg: RetargetingConfig) -> None:
    ...
    # ch_robot: the XML bodies named "left_*" are anatomically on the robot's RIGHT side
    # (see joint-mapping note in config_types/data_type.py). The retargeter's foot-sticking
