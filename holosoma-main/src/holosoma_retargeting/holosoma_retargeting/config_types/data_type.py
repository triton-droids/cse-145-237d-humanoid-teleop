...
JOINTS_MAPPINGS = {
    ...
    # ch_robot: 10-DOF lower-limb humanoid (no arms/waist)
    # Marker bodies are placed at joint pivot positions (spread in space), giving the interaction
    # mesh meaningful gradient information despite all physical body origins being co-located.
    #
    # NOTE: The XML bodies named "left_*" sit at x=0 and "right_*" at x=-0.31. The robot's
    # forward direction is +y (toe spheres at y=+0.08). By right-hand rule (forward=+y, up=+z),
    # the robot's anatomical LEFT is -x — so "right_*" bodies are actually on the robot's LEFT,
    # and "left_*" bodies are on the robot's RIGHT. Human L_* must map to "right_*" markers
    # (and vice versa) so the optimizer converges to a -90° base rotation (robot faces +x, walks
    # forward) rather than a +90° rotation (robot faces -x and walks sideways like a crab).
    ("smplh", "ch_robot"): {
        # Pelvis -> pelvis_marker (NOT torso): the torso body origin in the XML is
        # 0.577m BELOW the actual hip pivots, so mapping Pelvis to "torso" caused
        # geometric coupling between robot_height (scale factor) and standing height.
        # pelvis_marker is placed inside `hip` body at the midpoint of L/R hip pivots
        # (z=0.577 above torso origin), giving the retargeter a target that matches
        # the human pelvis position naturally.
        "Pelvis": "pelvis_marker",
        "L_Hip": "right_hip_marker",
        "R_Hip": "left_hip_marker",
        "L_Knee": "right_knee_marker",
        "R_Knee": "left_knee_marker",
        "L_Ankle": "right_ankle_marker",
        "R_Ankle": "left_ankle_marker",
        "L_Toe": "right_foot_sphere_3_link",
        "R_Toe": "left_foot_sphere_3_link",
    },
}
